...
class BertPredictionHeadTransform(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.pred_input_size, config.pred_input_size)
        if isinstance(config.hidden_act, str):
            self.transform_act_fn = ACT2FN[config.hidden_act]
        else:
            self.transform_act_fn = config.hidden_act
        self.LayerNorm = BertLayerNorm(config.pred_input_size, eps=config.layer_norm_eps)
...
class BertLMPredictionHead(nn.Module):
    def __init__(self, config):
        ...
        self.decoder = nn.Linear(config.pred_input_size, config.vocab_size, bias=False)
...
class SparseBertForMaskedLM(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.sparsity_imp = config.sparsity_imp
    ...
    def forward(
        ...
        sequence_output = torch.clamp(sequence_output, min=0, max=1)
        prediction_scores = self.cls(sequence_output)

        # Sparsity
        unfolded_output = sequence_output.reshape(sequence_output.size()[0] * sequence_output.size()[1], -1)
        target_sf = unfolded_output.new_full(unfolded_output[0].size(), fill_value=self.sparsity_frac)
        sparsity_loss = torch.sum(torch.clamp((unfolded_output.mean(axis=0) - target_sf), min=0) ** 2) + torch.mean(
            unfolded_output * (1 - unfolded_output))
        ...
        if masked_lm_labels is not None:
            ...
            outputs = (masked_lm_loss + (self.sparsity_imp * sparsity_loss),) + outputs
        ...
        if lm_labels is not None:
            ...
            outputs = (ltr_lm_loss + (self.sparsity_imp * sparsity_loss),) + outputs
...
class SparseHiddenBertForMaskedLM(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.bert = BertModel(config)
        self.cls = BertOnlyMLMHead(config)
    ...
    def forward(
        ...
        sparsity_loss = loss_recon(rec_outputs, all_outputs) + torch.sum(
...
MODEL_CLASSES = {
    'sp-bert': (SparseBertConfig, SparseBertForMaskedLM, BertTokenizer),
    'sp-bert-hidden': (SparseBertConfig, SparseHiddenBertForMaskedLM, BertTokenizer)
}
...
def mask_tokens(inputs, tokenizer, args):
    ...
    labels = inputs.clone()
    # We sample a few tokens in each sequence for masked-LM training (with probability args.mlm_probability defaults to
    # 0.15 in Bert/RoBERTa)
...
def main():
    ...
    parser.add_argument("--sparse_noise_std", default=.4, type=float,
                        help="The standard deviation of the gaussian noise in the sparse net.")
    parser.add_argument("--sparse_net_params", default="", type=str,
                        help="Path to pretrained sparse net weights.")
    ...
    if args.model_type == 'sp-bert':
        config.pred_input_size = config.hidden_size
    else:
        config.pred_input_size = config.sparse_size
